...
import {
  ...
  convertIndexToDateStr,
  convertIndexToDateObj,
} from "./dateFuncs";
const {
  getDaysInMonth,
  getDaysInYear,
...
const MonthView = ({ month, year, userDocs, editPastDate }: MonthViewProps) => {
  const daysToPrint =
    month === "-1"
      ? getDaysInYear(new Date(Number(year), Number(month)))
      : getDaysInMonth(new Date(Number(year), Number(month)));
  const firstWeekday =
    month === "-1"
      ? getDay(startOfMonth(new Date(Number(year), 0)))
      : getDay(startOfMonth(new Date(Number(year), Number(month))));
  ...
  if (month === "-1") {
    // Want to display full year.
    factor /= 1.5;
  }

  const moodColor = (index: number): string => {
    const today = new Date();
    const dateExists: boolean = userDocs.get(
    ...
    if (dateExists) {
      return `moods.${
        userDocs.get(convertIndexToDateStr(Number(year), Number(month), index))
          ?.mood! * 100
      }`;
    } else if (
      convertIndexToDateObj(Number(year), Number(month), index).getTime() <
      today.getTime()
    ) {
      return "transparent";
    } else {
            ...
            baseArr.map((index) => (
              ...
              >
                <Box
                  ...
                  borderColor="white"
                  borderRadius="10px"
                  key={`MonthView${convertIndexToDateStr(
                    Number(year),
                    Number(month),
                    Number(index)
                  )}`}
